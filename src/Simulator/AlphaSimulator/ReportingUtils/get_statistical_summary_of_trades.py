...
def get_statistical_summary_of_trades(trades_df, df_g=None, **params):
    ...
    for period, period_df, period_df_g in _split_df_into_pieces(
        trades_df, df_g, **params
    ):

        summary = {}
        summary["Market"] = params["market"] + f"-{period}"
        summary = add_statistical_reports(
            period_df, summary, period_df_g, is_chunk=True, **params
        )
...
def add_statistical_reports(df, summary, df_g=None, is_chunk=False, **params):
    ...
    if df_g is not None:
        ...
        if is_chunk:
            ...
            summary["annual(%)"] = (
                (1 + summary["return(%)"] / 100) ** (1 / years) - 1
            ) * 100

        ## TODO: ASSIGNMENT #1
        df_g.loc[:, "log_return"] = np.log(
            df_g["PortfolioValue"] / df_g["PortfolioValue"].shift(1)
        )
        mu = df_g["log_return"].mean()
        sigma = df_g.log_return.std()
        summary["annual_volatility"] = sigma * (252**0.5)
...
def _split_df_into_pieces(df, df_g, **params):
    ...
    if reporting_period == "Year" or df_g is None:
        ...
        for i in range(pieces):
            ...
            df_chunk = df[
                (df["opening_time"] >= chunk_start) & (df["opening_time"] <= chunk_end)
            ]

            if df_g is not None:
                df_g_chunk = df_g[
                    (df_g.index >= chunk_start) & (df_g.index <= chunk_end)
                ]
            else:
                df_g_chunk = None
        ...
        for period, period_df_g in split_df_g_s.items():
            ...
            yield period, period_df, period_df_g.drop(
                ["year", "fiscal_quarter", "month"], axis=1, errors="ignore"
            )
