...
class TradeHistoryHolder:
    ...
    def __init__(self, should_cache=True, **params):
        ...
        if should_cache:
            ...
            self.direc_original = os.path.join(
                self.enums.POTENTIAL_TRADES_DIR, f"{self.cache_key}-Original.csv"
            )

    def reset(self):
        self.history = deque[Trade]()
    ...
    def add(self, a_trade: Trade, market):
        self.history.append(a_trade)

    def add_many(self, list_of_trades: deque[Trade], market):
    ...
    def convert_trade_history_to_df(self):
        ...
        df = pd.DataFrame.from_records(holder)  # pd.DataFrame.from_dict(holder)
        report_df = df
    ...
    def load(self, df=None, force_load=False):
        ...
        try:
            ...
            if df is None:
                df = pd.read_csv(self.direc, index_col=0)
