...
from src.Simulator.AlphaSimulator.SimulatorUtils.Trade import Trade
...
def simulate_investment(
    history_of_trades: deque[Trade] | list[Trade],
    ...
    **params,
    ...
    for i, current_time in enumerate(dates_for_iteration):
        ...
        potential_trades = trades_by_time.get(
            current_time.replace(second=0, microsecond=0), None
        )
        ...
        for attempt in range(1):
            ...
            for i, trade in enumerate(potential_trades):
                ...
                trade, transaction_desc = open_a_new_position(
            ...
            potential_trades = [
                potential_trades[i]
                for i in range(len(potential_trades))
                if i not in indices_of_executed_trades
            ]
        ...
        for i, trade in enumerate(holder.active_trades_queue):
            ...
            if (
                are_equal(current_time, trade.closing_time, **params)
                and not trade.is_analyzed
            ):

                _analyze_trade_in_simulation(
                    trade, holder, candle_transactions, **params
                )

        # At the end of the candle, we should remove the completed trades
        holder.remove_completed_trades()
        locked_balance, net_exposure, long_positions_value, short_positions_value = (
            _get_unrealized_value_of_trades(
                holder.active_trades_queue, stock_histories, current_time, **params
            )
        )
        ...
        holder.handle_end_of_candle_reporting(
            ...
            locked_balance,
            net_exposure,
            long_positions_value,
            short_positions_value,
            candle_transactions,
        )
...
def should_skip_this_trade(
    trade, stock_histories, holder, current_time, next_valid_time, **params
):
    """
...
def open_a_new_position(
    trade, index_of_trade, indices_of_executed_trades, holder, **params
):
    """
    ...
    if (
        invested_budget_for_trade < holder.free_balance
        and holder.is_open_position_on_symbol[trade.symbol] is False
    ):
    ...
    net_exposure = get_net_exposure(
        holder.long_positions_value, holder.short_positions_value
    )
    holder.update_net_exposure(net_exposure)
...
def _mark_trade_analyzed_set_taxes_fess_return_gain(trade, **params):
    """
    ...
    capital_gain = (
        n_shares * (closing_price - trade.opening_price) * trade.trade_direction
    )
...
def _get_unrealized_value_of_trades(
    active_trades_queue, stock_histories, current_time, **params
):
    ...
    for trade in active_trades_queue:
        ...
        if not trade.is_analyzed:
            locked_balance_of_trade = (
                trade.invested_budget
                + trade_direction
                * (price_at_current_time - opening_price)
                * trade.n_shares
            )
            locked_balance += locked_balance_of_trade

            if trade_direction == LONG:
                long_positions_value += locked_balance_of_trade
            if trade_direction == SHORT:
                short_positions_value += locked_balance_of_trade
...
def _calculate_shares_and_investment_considering_other_trades(
    opening_price, budget, active_trades_queue=None, **params
):
    ...
    if active_trades_queue is None or len(active_trades_queue) == 0:
        ...
        min_of_invested_budget_in_other_trades = min(
            trade.invested_budget for trade in active_trades_queue
        )

    money_to_invest = max(
        money_to_invest_based_on_budget, min_of_invested_budget_in_other_trades
    )
...
class SimulationResultsHolder:
    ...
    def remove_completed_trades(self):
        self.active_trades_queue = _remove_completed_trades_from_trades_queue(
            self.active_trades_queue
        )
    ...
    def handle_end_of_candle_reporting(
        self,
        current_time,
        locked_balance,
        net_exposure,
        ...
        self.open_positions_at_the_end.append(
            " | ".join([trade.id for trade in self.active_trades_queue])
        )
        ...
        if self.market.startswith("US") or self.market.startswith("HK"):
            self.portfolio_value_at_close_yeasterday = (
                self.free_balance + locked_balance
            )
            ...
            raise NotImplementedError(
                f"This market {self.market} is not implemented yet"
            )
    ...
    def print_active_trades_queue(self):
        ...
        simulation_logger.warning(
            "These are the active trades remaining. Something should be wrong."
        )
...
def close_all_active_trades(
    current_time, holder, stock_histories, message, candle_transactions, **params
):
