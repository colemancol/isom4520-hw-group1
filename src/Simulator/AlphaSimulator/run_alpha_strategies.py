...
def run_alpha_strategies(
    ...
    all_market_stocks_dfs = list(market_and_df_interator(stock_histories, **params))
        ...
        while not all_market_stocks_dfs_chunk.empty():
            ...
            if should_log:
                print(
                    f"Simulating {macro_and_other_data['symbol']} in {market} market. "
                    + (
                        f"{all_market_stocks_dfs_chunk.qsize()} remaining. T: {time.time()-start:.2f}. "
...
def run_alpha_strategy_for_one_symbol(macro_and_other_data, params):
    ...
    should_limit_one_position_in_run_alpha = params[
    ...
    O(1) for the dict
    """
    dict_of_prices = df[["Open", "High", "Low", "Close", "Close_t_1"]].to_dict(
        orient="index"
    )
    ...
    for t in dict_of_signals:
        ...
        if (
            should_limit_one_position_in_run_alpha
            and latest_trade_closing_time is not None
            and t <= latest_trade_closing_time
        ):
            continue
        ...
        if (
            ...
            and not pd.isna(previous_close_price)
        ):
            ...
            stop_loss_threshold, take_profit_threshold = (
                set_stop_loss_and_take_profit_thresholds(
                    trade_opening_price, signal, **params
                )
            )
        ...
        while True:
            ...
            if t_monitoring < trade.exact_opening_time:
                continue
            ...
            if event_type == "stop_loss_at_open":
                close_trade_for_stop_loss(
                    trade, t_monitoring, open_price, "Stop Loss at Open", **params
                )
                ...
                close_trade_at(
                    trade, t_monitoring, close_price, "Hitting Close Signal", **params
                )
...
def _get_next_event(
    ...
    **params,
    ...
    if params["should_stop_loss"]:
        ...
        if len(filtered_df_for_sl_at_open) > 0:
            events.append(
                (
                    "stop_loss_at_open",
                    filtered_df_for_sl_at_open.index[0],
                    priorities_of_events["stop_loss_at_open"],
                )
            )
    ...
    if should_take_profit:
        ...
        if len(filtered_df_for_tp_at_open) > 0:
            events.append(
                (
                    "take_profit_at_open",
                    filtered_df_for_tp_at_open.index[0],
                    priorities_of_events["take_profit_at_open"],
                )
            )
    ...
    if should_stop_loss and trade.trade_direction == LONG:
        ...
        if len(filtered_df_for_sl_in_candle) > 0:
            events.append(
                (
                    "stop_loss_at_close",
                    filtered_df_for_sl_in_candle.index[0],
                    priorities_of_events["stop_loss_at_close"],
                )
            )
    ...
    if should_take_profit and trade.trade_direction == LONG:
        filtered_df_for_tp_in_candle = df[
            (df["Close"] > take_profit_threshold) & (not_same_candle_open_at_open)
        ]
        if len(filtered_df_for_tp_in_candle) > 0:
            events.append(
                (
                    "take_profit_at_close",
                    filtered_df_for_tp_in_candle.index[0],
                    priorities_of_events["take_profit_at_close"],
                )
            )
    ...
    if should_close_at_signal and trade.trade_direction == LONG:
        ...
        if len(filtered_df_for_close_signal) > 0:
            events.append(
                (
                    "hitting_close_signal",
                    filtered_df_for_close_signal.index[0],
                    priorities_of_events["hitting_close_signal"],
                )
            )
    ...
    # Max holding event
    filtered_df_for_max_holding = df[
        (df.index - trade.opening_time).days >= max_holding_days
    ]
    if len(filtered_df_for_max_holding) > 0:
        events.append(
            (
                "max_holding_days_touched",
                filtered_df_for_max_holding.index[0],
                priorities_of_events["max_holding_days_touched"],
            )
        )
    ...
    if should_close_at_end_of_candle:
        ...
        if len(filtered_df_for_end_of_candle) > 0:
            events.append(
                (
                    "close_at_end_of_candle",
                    filtered_df_for_end_of_candle.index[0],
                    priorities_of_events["close_at_end_of_candle"],
                )
            )
