...
class AllStocksPrices:
    ...
    def load(self):
        ...
        if market.startswith("US"):
            ...
            # Load the VIX index
            self.vix = load_data_from_yahoo_finance(
                "^VIX", market_macro_index=True, **params_to_pass
            )

            # Load the gold futures
            self.gold_futures = load_data_from_yahoo_finance(
                "GC=F", market_macro_index=True, **params_to_pass
            )

            # Load the 20 years treasuries
            self.treasuries_20years = load_data_from_yahoo_finance(
                "TLT", market_macro_index=True, **params_to_pass
            )

            # Load the russel 2000
            self.russel_2000 = load_data_from_yahoo_finance(
                "^RUT", market_macro_index=True, **params_to_pass
            )
            ...
            self.market_index = load_data_from_yahoo_finance(
                "^HSI", market_macro_index=True, **params_to_pass
            )
            macro_and_other_data = {"market_index": self.market_index}
        ...
        if len(self.data) > 0:
            ...
            for symbol, df_1d in self.data.items():
                ...
                df_1d, is_updated1 = add_statistical_measures(
                    df_1d,
                    macro_and_other_data=macro_and_other_data,
                    interval="1d",
                    **params_to_pass
                )
                df_1d, is_updated2 = add_technical_indicators(
                    df_1d,
                    macro_and_other_data=macro_and_other_data,
                    interval="1d",
                    **params_to_pass
