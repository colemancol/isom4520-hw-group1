...
class WinningRatePlotter:
    ...
    def _draw_winning_rate_for_weekdays(self):
        ...
        grouped = (
            self.df.groupby([col, "trade_direction"], observed=False)["is_successful"]
            .mean()
            .unstack()
        )
        ...
        if self.strategy_type == "market_neutral" and SHORT in grouped.columns:
            ...
            ax.bar(
                x2 + width / 2, grouped[SHORT], width=width, color="red", label="SHORT"
            )
    ...
    def _draw_winning_rate_for_opening_hours(self):
        ...
        # create the second set of bars (SHORT trades)
        if (
            self.strategy_type == "market_neutral"
            and ("mean", SHORT) in grouped.columns
        ):
            ax1.bar(
                x1 + width / 2,
                grouped[("mean", SHORT)],
    ...
    def _draw_stat_ratios(self):
        ...
        for col in cols_to_plot:
            try:
                ...
                self.df["ratio_group"] = pd.cut(self.df[col], bins=bins, labels=labels)
    ...
    def _draw_positive_stats(self):
        ...
        for col in cols_to_plot:
            ...
            bins, labels = generate_bins_labels_for_positive_values(
                self.df[col], num_bins=13
            )
    ...
    def _draw_histogram_of_pnl_ratios(self):
        ...
        # Plot the histograms
        ax1.hist(
            positive_df["PnL_ratio"],
            bins=20,
            alpha=0.5,
            range=bin_range,
            label="Trade Direction: 1",
        )
        ax2.hist(
            negative_df["PnL_ratio"],
            bins=20,
            alpha=0.5,
            range=bin_range,
            label="Trade Direction: -1",
...
def _draw_bar_chart(self, col):
    ...
    grouped = (
        self.df.groupby(["trade_direction", "ratio_group"], observed=False)[
            "is_successful"
        ]
        .mean()
        .unstack()
    ...
    positive_colors = list(
        plt.cm.Blues(np.linspace(0.2, 0.8, int(len(grouped.columns) / 2)))
    )
    negative_colors = list(
        plt.cm.Reds(np.linspace(0.2, 0.8, int(len(grouped.columns) / 2)))
    )
    ...
    # ------------------------------------------------------------------------ #
    total_counts = self.df.groupby(["trade_direction"], observed=False)[
        "ratio_group"
    ].count()
    count_grouped = (
        self.df.groupby(["trade_direction", "ratio_group"], observed=False)[
    ...
    # ------------------------------------------------------------------------ #
    grouped = (
        self.df.groupby(["trade_direction", "ratio_group"], observed=False)["PnL_ratio"]
        .mean()
        .unstack()
    )
...
def generate_bins_labels(data, num_bins):
    ...
    bins = [0]
    bins = (
        [-(i + 1) * bin_width for i in range(n)][::-1]
        + bins
        + [(i + 1) * bin_width for i in range(n)]
    )
